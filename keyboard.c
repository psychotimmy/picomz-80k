...
// --- Used only by the MZ-700
static bool alphashift=false;        // ALPHA key shifted status
...
// Used to send a repeating key to the MZ-80K/A/700
...
void mzrptkey(void)
{
  if (kaddr != 0xFF) {                // Check a keyboard is active
    ...
    if (rptcode && (to_ms_since_boot(get_absolute_time()) > rpttime)) {
      // Send the repeating key to the MZ-80K/A/700 and update next repeat time
      if (mzmodel == MZ80K)
...
// by mzhidmapkey<xxx>() rather than here
static void process_kbd_report(hid_keyboard_report_t const *report)
{
  ...
  if (mzmodel == MZ700) {
    // MZ-700 only - did the status of the Caps Lock key change ?
